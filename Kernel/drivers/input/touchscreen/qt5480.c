...
int qt5480_i2c_probe(struct i2c_client *client,const struct i2c_device_id *id);
static int __devexit qt5480_i2c_remove(struct i2c_client *client);
...
static struct i2c_driver g_qt5480_i2c_driver = {
    ...
    .probe      = qt5480_i2c_probe,
    .remove     = __devexit_p(qt5480_i2c_remove),
    .id_table   = qt5480_id,
};
...
static void qt5480_work_func(struct work_struct *aWork)
{
    ...
    if(ret < 0 )
    {
      PRINT_MSG("%s : i2c_read failed.", __func__);
      return;
    }
...
static irqreturn_t qt5480_irq_handler(int irq, void *dev_id)
    {
    ENTER_FUNC;
    disable_irq_nosync(g_qt5480_ts_driver->client->irq);
...
static int qt5480_probe(struct platform_device *aDevice)
    ...
    } while( !gpio_get_value(GPIO_TOUCH_INT_N) && (--n > 0) );

    if( n == 0 )
        PRINT_MSG("[QT5480]:[%s] TOUCH Interrupt recovery failed.\n", __func__);
    ...
    PRINT_MSG("[QT5480]:[%s] Before input_register_device() ...\n", __func__);
    ...
    if (ret == 0)
        {
        PRINT_MSG("[QT5480]:[%s] Start touchscreen %s\n", __func__, g_qt5480_ts_driver->input_dev->name);
        }
    else
        {
        PRINT_MSG("[QT5480]:[%s] Request_irq failed\n", __func__);
        }
...
{
	if( client == NULL ) {
	    printk( "%s : client is NULL !!", __func__ );
	    return -ENODEV;
	}
	
	g_qt5480_ts_driver->client = client;
	i2c_set_clientdata(client, g_qt5480_ts_driver);
	return 0;
}
...
static int __devexit qt5480_i2c_remove(struct i2c_client *client)
{
    ...
    g_qt5480_ts_driver = i2c_get_clientdata(client);
    kfree(g_qt5480_ts_driver);
    return 0;
}
...
static void init_hw_setting(void) {
    ...
    if( gpio_is_valid(GPIO_TOUCH_EN) ) {
        if( gpio_request( GPIO_TOUCH_EN, S3C_GPIO_LAVEL(GPIO_TOUCH_EN) ) )
            printk(KERN_ERR "Failed to request GPIO_TOUCH_EN!\n");
    ...
    if( gpio_is_valid(GPIO_TOUCH_RST) ) {
        if (gpio_request(GPIO_TOUCH_RST, S3C_GPIO_LAVEL(GPIO_TOUCH_RST)))
            printk(KERN_ERR "Failed to request GPIO_TOUCH_RST!\n");
