...
int device_poweron (void)
{
	if (gpio_is_valid(GPIO_BT_WAKE)) {
		if (gpio_request(GPIO_BT_WAKE, GPIO_LABEL(GPIO_BT_WAKE))) {
	...
	if (gpio_is_valid(GPIO_BT_RST_N)) {
		if (gpio_request(GPIO_BT_RST_N, GPIO_LABEL(GPIO_BT_RST_N))) {
...
int device_readhostline (int *hostval)
{
	if (gpio_is_valid(GPIO_BT_HOST_WAKE)) {
		if (gpio_request(GPIO_BT_HOST_WAKE, GPIO_LABEL(GPIO_BT_HOST_WAKE))) {
...
ssize_t btgpio_read (struct file *file, char __user *buf, size_t size, loff_t *off)
{
	int hostval = 0, ret;

	ret = device_readhostline(&hostval);
	if (ret)
		return ret;

	copy_to_user(buf, &hostval, sizeof(int));

	return 0;
}

long btgpio_ioctl( struct file *file, unsigned int ioctl_num, 
						unsigned long ioctl_param)
{
	...
	case IOCTL_BTREADSUSPEND: {
		...
		copy_to_user ((void *)ioctl_param,  &nHostVal, sizeof(int));
		break;
	}
	default:
		nretVal = -EINVAL;
 ...
 */
static int __init btgpioinit_module(void)
{
	int ret_val;
 ...
 */
static void __exit btgpioclean_module(void)
{
