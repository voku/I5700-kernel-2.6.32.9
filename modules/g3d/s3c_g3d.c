...
#define IS_DOMAIN_POWER_OFF regulator_is_enabled(g3d_regulator)
...
static long s3c_g3d_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	u32 val;
...
//		printk("\n\n====Success the malloc from kernel=====\n");
	...
	case S3C_3D_CACHE_INVALID:
		...
		dmac_map_area((const void *) param.vir_addr, param.size, PCI_DMA_BIDIRECTIONAL);
		mutex_unlock(&cache_invalid_lock);
		break;
	...
	case S3C_3D_CACHE_CLEAN:
		...
		dmac_unmap_area((const void *) param.vir_addr, param.size, PCI_DMA_BIDIRECTIONAL);
		mutex_unlock(&cache_clean_lock);
		break;
...
#ifdef CONFIG_S3C64XX_POWER_DOMAIN
	g3d_regulator = regulator_get(&pdev->dev, "pd");
	if(IS_ERR(g3d_regulator)) {
		printk(KERN_ERR PFX "failed to get power domain regulator\n");
		return -ENOENT;
	}
	DOMAIN_POWER_ON;
#endif /* CONFIG_S3C64XX_POWER_DOMAIN */
