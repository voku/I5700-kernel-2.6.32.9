...
static struct regulator *rot_domain;
...
static long s3c_rotator_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	ro_params	*params;
	ro_params	*parg;
	unsigned int mode;
...
struct file_operations s3c_rotator_fops = {
	...
	.compat_ioctl	= s3c_rotator_ioctl,
...
#ifdef CONFIG_S3C64XX_POWER_DOMAIN
	rot_domain = regulator_get(&pdev->dev, "pd");
	if (IS_ERR(rot_domain)) {
		printk(KERN_ERR "failed to get rotator power domain\n");
		return -ENOENT;
	}
	regulator_enable(rot_domain);
#endif /* CONFIG_S3C64XX_POWER_DOMAIN */
	rot_clock = clk_get(&pdev->dev, "rot");
	if(IS_ERR(rot_clock)) {
...
#ifdef CONFIG_S3C64XX_POWER_DOMAIN
#ifndef USE_ROTATOR_DOMAIN_GATING
	regulator_disable(rot_domain);
#endif
	regulator_put(rot_domain);
#endif
