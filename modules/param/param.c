...
#define BLOCK_OFFSET_MAIN	0
#define BLOCK_OFFSET_BACKUP	1
...
unsigned long param_get_crc(const unsigned char *mem, unsigned long size,
		      unsigned long crc)
{				/* calculate the crc value */
	crc = ~crc;
	while (size--)
		crc = (crc >> 8) ^ crc_table[(crc ^ *(mem++)) & 0xFF];

	return ~crc;
}
EXPORT_SYMBOL(param_get_crc);
...
static void write_param_block_callback(struct erase_info *self)
{
	struct completion *erase_completed = (struct completion *)self->priv;

	complete(erase_completed);
}
...
int write_param_block(unsigned int dev_id, unsigned char *addr, unsigned block_offset)
{
	...
	void *buf;
	struct mtd_info *mtd;
	DECLARE_COMPLETION_ONSTACK(erase_completed);
	...
	/* Allocate buffer for param table */
	buf = vmalloc(PARAM_LEN);
	if (!buf)
		return -ENOMEM;

	/* Get param mtd partition */
	mtd = get_mtd_device_nm("param");
	if (!mtd) {
		printk(KERN_ERR "%s: failed to get param mtd partition\n", __FUNCTION__);
		ret = -EINVAL;
		goto fail1;
	}

	if (mtd->erasesize_shift)
		start = block_offset << mtd->erasesize_shift;
	else
		start = block_offset * mtd->erasesize;

	/* Read current param table */
	ret = mtd->read(mtd, start, PARAM_LEN, &retlen, buf);
	if (ret || retlen != PARAM_LEN) {
		printk(KERN_ERR "%s: failed to read param block\n", __FUNCTION__);
		ret = -EIO;
		goto fail2;
	}
	...
	erase.callback = write_param_block_callback;
	erase.priv = (u_long)&erase_completed;

	/* Invoke erase command */
	mtd->erase(mtd, &erase);

	/* Wait for erase to complete */
	wait_for_completion(&erase_completed);
	...
	ret = mtd->write(mtd, start, PARAM_LEN, &retlen, buf);
	if (ret || retlen != PARAM_LEN) {
		printk(KERN_ERR "%s: write constant block failed\n", __FUNCTION__);
		ret = -EIO;
		goto fail2;
	};

	/* Write the variable block */
	ret = mtd->write(mtd, start + PARAM_LEN, PARAM_LEN, &retlen, addr);
	if (ret || retlen != PARAM_LEN) {
		printk(KERN_ERR "%s: write variable block failed\n", __FUNCTION__);
		ret = -EIO;
		goto fail2;
	}
...
int read_param_block(unsigned int dev_id, unsigned char *addr, unsigned block_offset)
{
	...
	/* Read current param table */
	ret = mtd->read(mtd, start + PARAM_LEN, PARAM_LEN, &retlen, addr);
	if (ret || retlen != PARAM_LEN) {
		printk(KERN_ERR "%s: failed to read param block\n", __FUNCTION__);
		ret = -EIO;
		goto fail2;
...
MODULE_AUTHOR("Jeonghwan Min <jeonghwan.min@samsung.com>");
MODULE_DESCRIPTION("Param Block Device Driver");
MODULE_LICENSE("GPL");
