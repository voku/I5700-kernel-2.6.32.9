...
static struct regulator *mfc_domain;
...
#define DOMAIN_POWER_ON do { \
	regulator_enable(mfc_domain); \
} while (0)

#define DOMAIN_POWER_OFF do { \
	regulator_disable(mfc_domain); \
} while (0)
...
static long s3c_mfc_ioctl(struct file *file,
...
static int s3c_mfc_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_S3C64XX_POWER_DOMAIN
	mfc_domain = regulator_get(&pdev->dev, "pd");
	if (IS_ERR(mfc_domain)) {
		printk(KERN_ERR "failed to get mfc power domain\n");
		return -ENOENT;
	}
	...
	// mfc clock enable
#ifdef CONFIG_PLAT_S3C64XX
	mfc_hclk = clk_get(&pdev->dev, "mfc_hclk");
	...
	mfc_sclk = clk_get(&pdev->dev, "mfc");
	if (!mfc_sclk || IS_ERR(mfc_sclk)) {
		printk(KERN_ERR "failed to get mfc clk source\n");
		return -ENOENT;
	}

	mfc_pclk = clk_get(&pdev->dev, "mfc_pclk");
...
#else
	// Set MFC clock to 133 MHz
	clk_set_rate(mfc_sclk, 133 * 1000 * 1000);
#endif
#endif /* CONFIG_PLAT_S5P64XX */
