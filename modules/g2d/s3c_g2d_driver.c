...
static struct regulator *g2d_regulator;
/* 
...
static long s3c_g2d_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	s3c_g2d_params	*params;
	ROT_DEG 		eRotDegree;
	int             ret = 0;
...
int s3c_g2d_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_S3C64XX_POWER_DOMAIN
	g2d_regulator = regulator_get(&pdev->dev, "pd");
	if (IS_ERR(g2d_regulator)) {
		printk(KERN_ERR "failed to get power domain regulator\n");
		return -ENOENT;
	}
#ifndef USE_G2D_DOMAIN_GATING
	regulator_enable(g2d_regulator);
#endif
#endif
	...
	s3c_g2d_clock = clk_get(&pdev->dev, "2d");
...
#ifdef CONFIG_S3C64XX_POWER_DOMAIN
#ifndef USE_G2D_DOMAIN_GATING
	regulator_disable(g2d_regulator);
#else
#ifdef USE_G2D_TIMER_FOR_CLK
        if(del_timer(&g_g2d_domain_timer))
		regulator_disable(g2d_regulator);
#endif /* USE_G2D_TIMER_FOR_CLK */
#endif
	regulator_put(g2d_regulator);
#endif
