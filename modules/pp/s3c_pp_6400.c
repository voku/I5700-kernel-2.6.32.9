...
static struct regulator *pp_domain;
...
#endif
#else
	printk(KERN_ERR "%s: PP FIFO mode unimplemented.\n", __FUNCTION__);
#endif
}
...
static long s3c_pp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	s3c_pp_instance_context_t *current_instance;
...
int s3c_pp_probe(struct platform_device *pdev)
{
	...
	int i;
#ifdef CONFIG_S3C64XX_POWER_DOMAIN
	pp_domain = regulator_get(&pdev->dev, "pd");
	if (IS_ERR(pp_domain)) {
		printk(KERN_ERR "failed to get power domain\n");
		return -ENOENT;
	}
	regulator_enable(pp_domain);
	...
	pp_hclk = clk_get(&pdev->dev, "post0_hclk");
	if (IS_ERR(pp_hclk)) {
		printk(KERN_ERR PFX "failed to find post hclk clock source\n");
		return -ENOENT;
	}
